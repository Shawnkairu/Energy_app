...
| Document | Purpose |
|----------|---------|
| [imported-specs/](./imported-specs/README.md) | **Source of truth** for product behavior — scenarios A–F + DRS/LBRS/go-live + AI-native system design |
| [SPEC_COMPLIANCE_CHECKLIST.md](./SPEC_COMPLIANCE_CHECKLIST.md) | Spec / IA / formula parity / stakeholder UX traceability — answers "does the product shape match the imported specs?" |
...
| [ARCHITECTURE.md](./ARCHITECTURE.md) | Surfaces, gates (DRS/LBRS) |
...
Use the checklist for merges that touch UX, APIs, or formulas. Use **this** doc when planning environments, integrations, demo labeling, and operational hardening.
...
**Staged launch** — A **controlled** deployment (limited buildings/users) with real operators, monitoring expectations, and any in-progress gaps tracked in [SPEC_COMPLIANCE_CHECKLIST.md](./SPEC_COMPLIANCE_CHECKLIST.md). Requires hardened secrets, backups, incident contacts, and explicit "non-production financial" posture unless counsel clears otherwise. The product spec is the imported scenarios — gaps in this tier are gaps to close, not carve-outs to ship with.

**Production** — General-availability operation: real money movement where applicable, measured energy where claimed, full auth/session posture, observability, DR/backups, jurisdiction-aware compliance (KYC/KYB, disclosures, escrow as required), and runbooks. Every imported-spec requirement satisfied.
...
| Area | Local prototype | Demo (synthetic) | Staged launch | Production | Current (repo) |
|------|-----------------|------------------|--------------|------------|----------------|
...
| **Ops / runbooks** | README commands | Demo reset procedure | On-call, rollback, DB restore drill | DR exercised | **stub** — README only; no ops playbook index |
| **Legal / compliance hooks** | N/A | Disclaimers in UX | Launch disclosures; no retail capital without counsel | KYC/KYB, escrow, jurisdiction gates | **partial** — UX/status placeholders; [COMPLIANCE_AND_RISK.md](./COMPLIANCE_AND_RISK.md) lists non-negotiables not fully implemented |
...
| **Cost / limits** | Unbounded OK | Cache external APIs (PVWatts, etc.) | Quotas documented | FinOps review | **partial** — free-tier limits documented inline; no centralized quota dashboard |
...
|----|----------|----------|------|---------|-------|------|
| DR-GAP-001 | High | Launch→prod | Legal / compliance | Financier flows surface **KYC/escrow/investment** UX as prototype; real KYC/KYB, custody, and disclosures are **not** implemented ([COMPLIANCE_AND_RISK.md](./COMPLIANCE_AND_RISK.md), scenario F traceability). | TBD | `docs/imported-specs/scenario-f-financier-flow.md` |
| DR-GAP-002 | Medium | Required | Data truth | **Synthetic** generation/load/irradiance per [SPEC_COMPLIANCE_CHECKLIST.md](SPEC_COMPLIANCE_CHECKLIST.md); measured-meter path is tracked in SPEC_COMPLIANCE_CHECKLIST. Risk: demos mistaken for ground truth without badges/copy. | Eng | `mobile/components/PilotBanner.tsx`, shared `source` / `dataSource` types |
...
| DR-GAP-004 | Medium | Prod auth | Auth | **Email OTP** and dev/console OTP paths are operational; **SMS** and carrier governance are required. | Eng | [SPEC_COMPLIANCE_CHECKLIST.md](SPEC_COMPLIANCE_CHECKLIST.md) |
...
Passing CI means: workspace **typecheck**, **lint**, **build**, **shared tests + audit**, backend **migrations + pytest** against Postgres. That validates **engineering correctness** on a clean checkout — not production readiness, tenant penetration testing, or legal sign-off. Treat CI as a **subset** of the Staged launch / Production columns above.
