...
This is a long-arc strategy doc, not a product spec. Canonical product behavior is in [docs/imported-specs/](imported-specs/README.md); this doc captures *where the company is going* so we don't lose the thread.
...
The product spec is what's in [imported-specs/](imported-specs/README.md). Six public roles, apartment buildings and homeowners both first-class. The current build is in motion; gaps between code and the imported specs are tracked in [SPEC_COMPLIANCE_CHECKLIST.md §3.2](SPEC_COMPLIANCE_CHECKLIST.md) and closed forward, not deferred. Operational maturity per environment lives in [DEPLOYMENT_AND_READINESS.md](DEPLOYMENT_AND_READINESS.md).

How we *talk* about what we're shipping matters: we're not "an apartment-building solar app with homeowner support added on." We're "a coordination network for distributed infrastructure, with two initial deployment topologies." Marketing copy and investor pitch should reflect the larger framing.

## What this means after MVP launch

Three things become priorities once MVP validates:
...
3. **Scope-creep on the product team.** "We're a coordination network!" can become "let's build everything." Anchoring to the imported scenario specs ([imported-specs/](imported-specs/README.md)) becomes more important, not less, as the framing gets bigger.
...
The core rules in [README.md](../README.md) and product doctrine in [PRODUCT_SPEC.md](PRODUCT_SPEC.md) hold. Prepaid (with pre-activation pledges as the demand signal per scenario A §5), DRS gates everything, monetized solar drives payouts, ownership ledger sums to 100%, settlement reconciles to the cent. The bigger framing doesn't change the discipline; it raises the stakes.

## When to revisit this doc

After MVP ships and runs against real users for ≥30 days. Re-examine:
...
**Discipline reminder:** This doc captures the long arc. Shipping the imported-spec product is what makes the long arc possible. Don't let strategy talk eat shipping time.
